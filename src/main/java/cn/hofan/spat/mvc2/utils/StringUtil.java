	...
	public static final String FOLDER_SEPARATOR = "/";
	  ...
	  public static String capitalize(CharSequence s)
	  {
	    ...
	    if (Character.isUpperCase(char0))
	      return s.toString();
	    return len + Character.toUpperCase(char0) +  s.subSequence(1, len).toString();
	  }
	  ...
	  public static String lowerFirst(CharSequence s)
	  {
	    ...
	    if (Character.isLowerCase(c))
	      return s.toString();
	    return len + Character.toLowerCase(c) + s.subSequence(1, len).toString();
	  }
	  ...
	  public static String[] splitIgnoreBlank(String s, String regex)
	  {
	    ...
	    int j = (arrayOfString1 = ss).length; for (int i = 0; i < j; ++i) { String st = arrayOfString1[i];
	      if (isBlank(st))
	        continue;
	      list.add(trim(st));
	    }
	  ...
	  public static int maxLength(Collection<? extends CharSequence> coll)
	  {
	    int re = 0;
	    if (coll != null)
	      for (Iterator localIterator = coll.iterator(); localIterator.hasNext(); ) { CharSequence s = (CharSequence)localIterator.next();
	        if (s == null)  break;
	        label41: re = Math.max(re, s.length()); }
	    return re;
	  }
	  ...
	  public static <T extends CharSequence> int maxLength(T[] array)
	  {
	    ...
	    if (array != null) {
	      ...
	      int j = (arrayOfCharSequence = array).length; for (int i = 0; i < j; ++i) { CharSequence s = arrayOfCharSequence[i];
	        if (s != null)
	  ...
	  public static boolean isin(String[] ss, String s)
	  {
	    ...
	    if ((ss == null) || (ss.length == 0) || (isBlank(s)))
	      return false;
	    int j = (arrayOfString = ss).length; for (int i = 0; i < j; ++i) { String w = arrayOfString[i];
	      if (s.equals(w))
	        return true; 
	    }
	    return false;
